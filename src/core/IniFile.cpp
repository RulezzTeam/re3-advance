...
float CIniFile::PedNumberMultiplier = 100.6f;
float CIniFile::CarNumberMultiplier = 100.6f;
...
void CIniFile::LoadIniFile()
{
	...
	if (f){
		...
		PedNumberMultiplier = Min(300.0f, Max(0.5f, PedNumberMultiplier));
		CFileMgr::ReadLine(f, gString, 200);
		sscanf(gString, "%f", &CarNumberMultiplier);
		CarNumberMultiplier = Min(300.0f, Max(0.5f, CarNumberMultiplier));
