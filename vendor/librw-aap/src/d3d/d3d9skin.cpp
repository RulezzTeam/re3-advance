...
// Per-pixel lighting variants
static void *skin_pp_amb_VS;
...
//		assert(skin_all_VS);
	...
	// Per-pixel variants. These shed the lighting loop from the VS, so they
	// have plenty of headroom even with skinning.
	{
		static
#include "shaders/skin_pp_amb_VS.h"
		skin_pp_amb_VS = createVertexShader((void*)VS_NAME);
		assert(skin_pp_amb_VS);
	}
	{
		static
#include "shaders/skin_pp_amb_dir_VS.h"
		skin_pp_amb_dir_VS = createVertexShader((void*)VS_NAME);
		assert(skin_pp_amb_dir_VS);
	}
	{
		static
#include "shaders/skin_pp_all_VS.h"
